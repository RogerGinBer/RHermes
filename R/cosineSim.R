...
cosineSim <- function(pattern, query, nscans = 5) {
    sametime <- which(query$rt %in% pattern$rt)  #Only scans found on both pattern and query are selected
    if (length(sametime) < nscans) {
        return(0)
    }
    ...
    # return(sum((pattern$rtiv - mean(pattern$rtiv)) * (query$rtiv - mean(query$rtiv)))/
    #          sqrt(sum((pattern$rtiv - mean(pattern$rtiv))  ^ 2)
...
MSMScosineSim <- function(pattern, query, minhits = 1, mode = "full") {
    ...
    if (mode == "full") {
        denom <- sqrt(sum(pattern$int^2)) * sqrt(sum(query[, 
            2]^2))
    } else {
