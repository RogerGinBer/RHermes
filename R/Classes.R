...
#' @slot nthr The noise threshold to consider. Any signal weaker than it will NOT
...
#' @slot ion Polarity used in the experiment. RHermes currently only supports one polarity
...
#' @slot instr MS instrument type used to acquire the data. It can be either 'QTOF' or 'Orbitrap'.
...
RHermesIdent <- setClass("RHermesIdent", slots = list(IdentifiedSOI = "data.table",
...
setRHermesCluster <- function(){
    if(Sys.info()[1] == "Windows"){
        ...
        return(BiocParallel::MulticoreParam(BiocParallel::multicoreWorkers()-2,
                                     progressbar = TRUE))
    }
}


#'The metadata storage class. It holds the experimental parameters, all the names
#'of the processed files and the timestamps for all operations performed on the
#'parental RHermesExp object
#'@slot ExpParam  Contains all experimental information (ppm error, resolution, polarity, etc.),
#'as well as the formula and adduct databases used. See ExpParam for more info.
#'@slot filenames All filenames of the processed files. This info is also available
#' for individual PL and SOI objects in their respective slot.
#'@slot timestamps Timestamps generated when any operation was performed on the parental object.
#'You can easily check them with readTime().
...
#'
#'@slot PL A list object that holds all RHermesPL objects containing a PL each. See RHermesPL for more info
#'@slot SOI List that holds RHermesSOI objects with a SOI list inside each. See RHermesSOI for more info.
#'@slot MS2Exp List to hold inclusion lists, MSMS experimental data and compound identifications.
#'@slot Quant List that will hold compound quantifications (in dev).
#'
...
#' you need. All main RHermes functions use it and return an updated version of
#' the object.
#'
#' @slot metadata Where all the complementary info is stored (experimental
#' parameters, timestamps, databases, etc.). See RHermesMeta for more info.
#' @slot data All experimental info is stored in data. It is divided into PL (peaklist)
#' SOI (scans of interest), MS2Exp (for the MS/MS experiments and identifications) and
#' Quant (for quantifications, still not developed). See RHermesData for more info.
#' @export RHermesExp
#' @examples
#' if(FALSE){
