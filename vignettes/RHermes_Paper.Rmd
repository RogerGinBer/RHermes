...
output: 
  html_document:
    ...
    toc_float: true
vignette: |
  %\VignetteIndexEntry{RHermes Paper} %\VignetteEngine{knitr::rmarkdown}
---
...
for(i in 1:nrow(comp_list)){
  ...
  if(comp_list[i, 3] == "[M+H]+"){
    plot <- RHermes:::SoiPlot(waterPos, 5, formula = comp_list$Formula[i], 
              rtrange = c(t-tol,t+tol), dynamicaxis = TRUE,
...
waterPosUnfiltered <- readRDS("D:/HermesResults/SurfaceWater/PosSOI.rds")
chloridazon <- "[C10H9ClN3O]+"
RHermes::IsoFidelity(waterPosUnfiltered, soilist = 6,
                     entry = which(waterPosUnfiltered@data@SOI[[6]]@SoiList$formula == chloridazon)[2], plot = TRUE)

alphaC <- "[C11H9KN3]+"
RHermes::IsoFidelity(waterPosUnfiltered, soilist = 6,
                     entry = which(waterPosUnfiltered@data@SOI[[6]]@SoiList$formula == alphaC)[2], plot = TRUE)

m1 <- with(waterPosUnfiltered@data@SOI[[6]]@SoiList, mass[formula == chloridazon])[2]
m2 <- with(waterPosUnfiltered@data@SOI[[6]]@SoiList, mass[formula == alphaC])[2]
cat("Mass difference: ", round(m1-m2, digits = 5), " Da\n",
             "  ppm distance: ", (m1-m2)/m1*1e6, "\n")
```

### ISF annotation
```{r}
DB <- readRDS("D:/MS2ID_B2R_20201113_083214.rds")
anotatedSL <- RHermes:::anotateISF(waterPosUnfiltered@data@SOI[[6]]@SoiList, DB, polarity = 1,
                                   BiocParallelParam = SnowParam(4, progressbar = TRUE))


```

### Ionic formula overlap
```{r}
...
enrichPos <- lapply(seq_along(ecoliPos13C@data@SOI), function(soi){
  ...
  do.call("rbind", lapply(1:nrow(ecoliPos13C@data@SOI[[soi]]@SoiList), function(i){
    tryCatch(enrichment(ecoliPos13C, soi, i, FALSE), error = function(cond){return(c(0,0))})
  }))
})
...
enrichNeg <- lapply(seq_along(ecoliNeg13C@data@SOI), function(soi){
  ...
  do.call("rbind", lapply(1:nrow(ecoliNeg13C@data@SOI[[soi]]@SoiList), function(i){
    tryCatch(enrichment(ecoliNeg13C, soi, i, FALSE), error = function(cond){return(c(0,0))})
  }))
})
