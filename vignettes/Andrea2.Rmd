...
myHermes <- RHermesExp()
myHermes <- setExpParam(myHermes, template = "orbi-pos", params = ExpParam(ppm = 2, instrument = "Orbitrap", res = "120000"))
```
...
setwd("../") #For unknitted use of the chunk
myHermes <- setDB(myHermes, "norman") #Default HMDB, single charged, monomeric adducts
readTime(myHermes)
```
...
dir <- "D:/ABrunner"
fil <- list.files(dir, pattern = ".*pos.*.mzML")[c(1,4,7)]
fil <- paste(dir, fil, sep = "/")
...
setwd("../") #For unknitted use of the chunk
s <- getSOIpar("double")
myHermes <- SOIfinder(myHermes, params = s, fileID = c(2,3),
                      against = c(1,2))
...
```{r}
copy <- myHermes #Just in case
copy <- SOIcleaner(copy, 2, 10000, TRUE)
```
...
```{r}
copy <- genIL(copy, id = 2, par = ILParam(priorization = "yes", ad = c("M+H", "M+NH4")))

myHermes <- genIL(myHermes, id = 2, par = ILParam())
```
...
```{r}
ms2f <- list.files("D:/ABrunner/MSMSdata", pattern = ".*pos.*.mzML")[seq(33,45)]
ms2f <- paste0("D:/ABrunner/MSMSdata/", ms2f)
