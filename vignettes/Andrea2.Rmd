...
```{r , warning=FALSE}
myHermes <- readRDS("D:/ABrunner/MS1_pos_PL.rds")
setwd("../") #For unknitted use of the chunk
s <- getSOIpar("double")
myHermes <- SOIfinder(myHermes, params = s, fileID = c(1,2,2,3,3,3),
                      against = c(0,0,1,0,1,2))
saveRDS(myHermes, file = "D:/ABrunner/MS1_pos_SOI.rds")
...
saveRDS(myHermes, file = "D:/ABrunner/MS1_pos_SOI_filter.rds")
```

```{r, warning=FALSE}
negHermes <- readRDS("D:/ABrunner/MS1_neg_PL.rds")
s <- getSOIpar("double")
negHermes <- SOIfinder(negHermes, params = s, fileID = c(1,2,2,3,3,3),
                      against = c(0,0,1,0,1,2))
saveRDS(negHermes, file = "D:/ABrunner/MS1_neg_SOI.rds")

for(i in 1:6){
  negHermes <- SOIcleaner(negHermes, soiid = i, minint = 10000, TRUE)
}
saveRDS(negHermes, file = "D:/ABrunner/MS1_neg_SOI_filter.rds")
```
...
myHermes <- MS2Proc(myHermes, 1, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)
myHermes <- MS2Proc(myHermes, 2, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)


ms2f <- list.files("D:/ABrunner/MSMSdata", pattern = ".*pos.*.mzML", full.names = TRUE)[seq(33,45)]
myHermes <- MS2Proc(myHermes, 3, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)
myHermes <- MS2Proc(myHermes, 4, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)

ms2f <- list.files("D:/ABrunner/MSMSdata", pattern = ".*pos.*.mzML", full.names = TRUE)[c(1:31, seq(33,45))]
myHermes <- MS2Proc(myHermes, 5, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)
myHermes <- MS2Proc(myHermes, 6, MS2files = ms2f, referenceDB = "D:/MS2ID_20200824_202808.rds", useDB = TRUE, mincos = 0.5)

saveRDS(myHermes, file = "D:/ABrunner/MS1_pos_Ident_Water2.rds")
...
negHermes <- MS2Proc(negHermes, 1, MS2files = ms2f)
negHermes <- MS2Proc(negHermes, 2, MS2files = ms2f)


ms2f <- list.files("D:/ABrunner/MSMSdata", pattern = ".*neg.*.mzML", full.names = TRUE)[seq(35,45)]
negHermes <- MS2Proc(negHermes, 3, MS2files = ms2f)
negHermes <- MS2Proc(negHermes, 4, MS2files = ms2f)
saveRDS(negHermes, file = "D:/ABrunner/MS1_neg_Ident_Water.rds")
