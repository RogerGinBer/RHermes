...
myHermes <- MS2Proc(myHermes, 1, MS2files = ms2f)
myHermes <- MS2Proc(myHermes, 2, MS2files = ms2f)


ms2f <- list.files("D:/ABrunner/MSMSdata", pattern = ".*pos.*.mzML", full.names = TRUE)[seq(33,45)]
myHermes <- MS2Proc(myHermes, 3, MS2files = ms2f)
myHermes <- MS2Proc(myHermes, 4, MS2files = ms2f)
saveRDS(myHermes, file = "D:/ABrunner/MS1_pos_Ident.rds")
...
saveRDS(negHermes, file = "D:/ABrunner/MS1_neg_Ident.rds")
